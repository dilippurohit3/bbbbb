...
export class Room {
  ...
  public netflixSession: {
    isActive: boolean;
    videoUrl: string;
    title: string;
    participantsReady: Set<string>;
    totalParticipants: number;
  } = {
    isActive: false,
    videoUrl: '',
    title: '',
    participantsReady: new Set(),
    totalParticipants: 0
  };
  ...
  constructor(
    ...
    io.of(roomId).on('connection', (socket: Socket) => {
      ...
      socket.on('netflix:startSession', (data) => this.startNetflixSession(socket, data));
      socket.on('netflix:stopSession', () => this.stopNetflixSession(socket));
      socket.on('netflix:markReady', () => this.markNetflixReady(socket));
      socket.on('netflix:syncPlay', () => this.syncNetflixPlay(socket));
      socket.on('netflix:syncPause', () => this.syncNetflixPause(socket));
      socket.on('netflix:syncSeek', (data) => this.syncNetflixSeek(socket, data));
      socket.on('netflix:changeVideo', (data) => this.changeNetflixVideo(socket, data));
  ...
  private disconnectUser = (socket: Socket) => {
    ...
    // Clean up Netflix session if user was ready
    if (this.netflixSession.isActive && this.netflixSession.participantsReady.has(socket.id)) {
      this.netflixSession.participantsReady.delete(socket.id);
      this.netflixSession.totalParticipants = this.roster.length;
      
      // Broadcast updated ready count
      this.io.of(this.roomId).emit('netflix:participantLeft', {
        participantId: socket.id,
        readyCount: this.netflixSession.participantsReady.size,
        totalCount: this.netflixSession.totalParticipants
      });
    }
  ...
  // Netflix sync methods
  private startNetflixSession = (socket: Socket, data: { videoUrl: string; title: string }) => {
    if (!this.validateLock(socket.id)) {
      return;
    }
    ...
    this.netflixSession = {
      ...
      title: data.title,
      participantsReady: new Set(),
      totalParticipants: this.roster.length
    };

    // Broadcast to all participants
    this.io.of(this.roomId).emit('netflix:startSession', {
      videoUrl: data.videoUrl,
      title: data.title,
      instructions: [
    ...
    const chatMsg = {
      id: socket.id,
      cmd: 'netflixStart',
      msg: `Started Netflix session: ${data.title}`
    };
    this.addChatMessage(socket, chatMsg);
  };

  private stopNetflixSession = (socket: Socket) => {
    if (!this.validateLock(socket.id)) {
      return;
    }

    this.netflixSession.isActive = false;
    this.netflixSession.participantsReady.clear();
    this.netflixSession.totalParticipants = 0;

    this.io.of(this.roomId).emit('netflix:stopSession');
    
    const chatMsg = {
      id: socket.id,
      cmd: 'netflixStop',
      msg: 'Stopped Netflix session'
    };
    this.addChatMessage(socket, chatMsg);
  };

  private markNetflixReady = (socket: Socket) => {
    if (!this.netflixSession.isActive) {
      return;
    }

    this.netflixSession.participantsReady.add(socket.id);
    
    // Broadcast to all participants
    this.io.of(this.roomId).emit('netflix:participantReady', {
      participantId: socket.id,
      readyCount: this.netflixSession.participantsReady.size,
      totalCount: this.netflixSession.totalParticipants
    });
    ...
    if (this.netflixSession.participantsReady.size === this.netflixSession.totalParticipants) {
      this.io.of(this.roomId).emit('netflix:allReady');
    }
  };

  private syncNetflixPlay = (socket: Socket) => {
    if (!this.validateLock(socket.id) || !this.netflixSession.isActive) {
      return;
    }

    this.io.of(this.roomId).emit('netflix:syncPlay');
    
    const chatMsg = {
      id: socket.id,
      cmd: 'netflixPlay',
      msg: 'Played Netflix for all'
    };
    this.addChatMessage(socket, chatMsg);
  };

  private syncNetflixPause = (socket: Socket) => {
    if (!this.validateLock(socket.id) || !this.netflixSession.isActive) {
      return;
    }

    this.io.of(this.roomId).emit('netflix:syncPause');
    
    const chatMsg = {
      id: socket.id,
      cmd: 'netflixPause',
      msg: 'Paused Netflix for all'
    };
    this.addChatMessage(socket, chatMsg);
  };

  private syncNetflixSeek = (socket: Socket, data: { timestamp: number }) => {
    if (!this.validateLock(socket.id) || !this.netflixSession.isActive) {
      return;
    }

    this.io.of(this.roomId).emit('netflix:syncSeek', { timestamp: data.timestamp });
    
    const chatMsg = {
      id: socket.id,
      cmd: 'netflixSeek',
      msg: `Seeked to ${data.timestamp}s`
    };
    this.addChatMessage(socket, chatMsg);
  };

  private changeNetflixVideo = (socket: Socket, data: { videoUrl: string; title: string }) => {
    if (!this.validateLock(socket.id) || !this.netflixSession.isActive) {
      return;
    }

    this.netflixSession.videoUrl = data.videoUrl;
    this.netflixSession.title = data.title;
    this.netflixSession.participantsReady.clear();

    this.io.of(this.roomId).emit('netflix:videoChanged', {
      videoUrl: data.videoUrl,
      title: data.title
    });
